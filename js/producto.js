...
fetch("data/productos.json")
  ...
  .then(productos=>{
    ...
    if(!producto){ contenedor_detalle.innerHTML="<p>Producto no encontrado</p>"; return; }
...
function crear_talles(producto){
  ...
  Object.entries(producto.talles).forEach(([talle,colores])=>{
    ...
    div_talle.className="talle"; div_talle.textContent=talle;
    if(colores.length===0) div_talle.classList.add("agotado");
    else div_talle.addEventListener("click",()=>{
